...
<html>
    ...
    <body>
        ...
        while ($resultado = mysql_fetch_assoc($query)) {
            ...
            <a href="Restrito.php?PAGINA=Controladorvendas&&codVendas=<?php echo $COD; ?>" id="mensagemenviarparaentrega" style="height: 80px;line-height: 80px;font-size: 22px;color: white;position: absolute;top: 50%;left: 50%;width: 700px;margin-top: -40px;border-radius: 3px;margin-left: -350px;">
                Enviar para entrega
            </a>
    ...
    <?php }
    ?>
    <div style="width: 500px;height: 45px;line-height: 45px;background: white;color: #333;position: absolute;top: 50px;left: 50%;margin-left: -250px;border-radius: 3px;box-shadow: 0 0 10px rgba(0,0,0, .2)">
        <?php echo $count; ?> vendas pendentes
    </div>

    <?php
    if (isset($_SESSION['MENSAGEMENVIARCOMPLETO'])) {
        if (($_SESSION['MENSAGEMENVIARCOMPLETO']) == 1) {
            ?>
            ...
            <div id="msglogin" style="display: none;width: 500px;height: 100px;line-height: 100px;position: absolute;top: 50%;left: 50%;margin-top: -50px;margin-left: -250px;z-index: 15;background: white;border-radius: 3px;box-shadow: 0 0px 10px rgba(0, 0, 0, 0.2);">
                <p style="margin:0;padding: 0;font-size: 18px;color: #333;">Envio realizado com sucesso!</p>
            </div>

            <?php $_SESSION['MENSAGEMENVIARCOMPLETO'] += 1; ?> 
            <?php
        }
    }
    if (isset($_SESSION['MENSAGEMENVIARCOMPLETO'])) {
        if ($_SESSION['MENSAGEMENVIARCOMPLETO'] == 2) {
            unset($_SESSION['MENSAGEMENVIARCOMPLETO']);
        }
    }
    ?> 
